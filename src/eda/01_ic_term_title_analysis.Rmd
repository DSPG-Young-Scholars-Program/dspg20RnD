...
tidy_titles <- tibble(dept = project_titles$DEPARTMENT, 
...
important_words <- function(df, department) {
  selected <- df %>% filter(dept == department)
  selected %>%
    arrange(desc(tf_idf)) %>%
...
abstracts <- raw_abstracts %>%
    select(DEPARTMENT, ABSTRACT)

tidy_abstracts <- tibble(dept = abstracts$DEPARTMENT, text = abstracts$ABSTRACT)
...
important_words(tidy_abstracts, c("VA", "EPA", "HHS", "NASA")) + labs(y = "abstract")
important_words(tidy_terms, c("DOD", "EPA", "HHS", "NASA")) + labs(y = "terms")
important_words(tidy_titles, c("DOD", "EPA", "HHS", "NASA")) + labs (y = "titles")
